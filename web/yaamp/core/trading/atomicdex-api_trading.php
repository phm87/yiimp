<?php
 function doAtomicDEXCancelOrder($OrderID=false)
{
	if(!$OrderID) return;
 	// todo
	 return;
	 
        $res = atomicdex_api_user("cancel_order", array("uuid" => $OrderID));
        if($res && !isset($res->error)) {
                $db_order = getdbosql('db_orders', "market=:market AND uuid=:uuid", array(
                        ':market'=>'AtomicDEX', ':uuid'=>$OrderID
                ));
 ...
 function doAtomicDEXTrading($quick=false)
{
	...
	$updatebalances = true;
 	...
 	$list = atomicdex_api_user('get_enabled_coins');
        if (empty($list)) return;
 	$savebalance = getdbosql('db_balances', "name='$exchange'");
 	foreach($data as $balance)
	{
                $data = atomicdex_api_user('my_balance', array('coin' => $c->ticker));
                if (empty($data)) return;
                if ($c->ticker == 'BTC') {
                        if (is_object($savebalance)) {
                                $savebalance->balance = $data->balance;
                                $savebalance->onsell = $data->locked_by_swaps;
                                $savebalance->save();
                        }
                        continue;
                }
 		if ($updatebalances) {
			...
			$coins = getdbolist('db_coins', "symbol=:symbol OR symbol2=:symbol",
				array(':symbol'=>$c->ticker))
			);
			if (empty($coins)) continue;
			foreach ($coins as $coin) {
				$market = getdbosql('db_markets', "coinid=:coinid AND name='$exchange'", array(':coinid'=>$coin->id));
				if (!$market) continue;
				$market->balance = $data->balance;
                                $market->ontrade = $data->locked_by_swaps;
                                $market->balancetime = time();
                                $market->deposit_address = $c->address;
				$market->save();
			}
		}
	}
	 ...
	 return;
		...
		$market = getdbosql('db_markets', "coinid=:coinid AND name='zebitex'", array(':coinid'=>$coin->id));
			...
			debuglog("pair is ".$pairs[0]);
                        ...
                        {
                                debuglog("AtomicDEX: cancel order $pair at $sellprice, ask price is now $ask");
                                sleep(1);
                                doAtomicDEXCancelOrder($order->id);
                        }
                        // store existing orders
                        else
			{
			                                $db_order = getdbosql('db_orders', "market=:market AND uuid=:uuid", array(
                                        ':market'=>'AtomicDEX', ':uuid'=>$order->id
                                ));
                                if($db_order) continue;
                                 debuglog("AtomicDEX: store order of {$order->amount} {$symbol} at $sellprice BTC");
                                $db_order = new db_orders;
                                $db_order->market = 'AtomicDEX';
                ...
                $list = getdbolist('db_orders', "coinid={$coin->id} AND market='AtomicDEX'");
                foreach($list as $db_order)
			{
                        ...
                        if(!$found) {
                                 debuglog("AtomicDEX: delete db order {$db_order->amount} {$coin->symbol} at {$db_order->price} BTC");
                                $db_order->delete();
                        }
                }
...
//  {"asks":[["0.01654547","3.0"],["0.01784895","2.654466"],["0.018799","4.0"],["0.01954657","0.345534"]],"bids":[["0.01","2.0"],["0.001","11.0"]]}
                ...
                for($i = 0; $i < 5 && $amount >= 0; $i++)
                {
                        ...
                        debuglog("AtomicDEX: selling $sellamount $symbol at $sellprice");
                        sleep(1);
			$params = array('bid' => 'btc', 'ask' => strtolower($symbol), 'side' => 'ask', 'price' => $sellprice,
                        ...
                        if(!$res || isset($res->error)) {
                                debuglog("AtomicDEX create order err: ".json_encode($res));
                                break;
                        }
                        $amount -= $sellamount;
                }
                ...
                debuglog("AtomicDEX: selling $amount $symbol at $sellprice");
                sleep(1);
                $params = array('bid' => 'btc', 'ask' => strtolower($symbol), 'side' => 'ask', 'price' => $sellprice,
                                'amount'=>$sellamount, 'market' => strtolower($symbol).'btc', 'ord_type' => 'limit');
                $res = atomicdex_api_user('orders', $params);
                if(!$res || isset($res->error)) {
                        debuglog("AtomicDEX create order err: ".json_encode($res));
                        continue;
                }
...
//              $db_order->uuid = $res->id;
        ...
        if(floatval($withdraw_min) > 0 && $savebalance->balance >= ($withdraw_min + $withdraw_fee))
        {
                ...
                debuglog("AtomicDEX: withdraw $amount BTC to $btcaddr");
                sleep(1);
                $params = array("code" => $amount."btc");
                $res = atomicdex_api_user('withdrawals', $params);
                if(is_object($res) && !isset($res->error))
                {
			$withdraw = new db_withdraws;
                        $withdraw->market = 'AtomicDEX';
...
//                        $withdraw->uuid = $res->Data;
                        ...
                        debuglog("AtomicDEX withdraw BTC error: ".json_encode($res));
                }
        }
 }
