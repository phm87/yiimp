...
class ExchangeCommand extends CConsoleCommand
{
	...
	public function testApiKeys()
	{
		...
		if (!empty(EXCH_ATOMICDEX_RPCPASSWORD)) {
			$balance = atomicdex_api_user('my_balance', array("coin"=>"BTC"));
			if (!isset($balance->balance)) echo "AtomicDEX error ".json_encode($balance)."\n";
			else echo("AtomicDEX: BTC balance: ".json_encode($balance->balance)."\n");
		}
		// only one secret key
